...
class ParameterSpecification:
    ...
    def __init__(self, *args, **kwargs):
        ...
        **kwargs - key-value attributes
        ...
        if args:
            if len(args) != 1:
                raise BIException('Constructor of <%s> expect one no-named parameter only!' % self.__class__.__name__)
    ...
    def validate_value(self, value):
        ...
        if self.param_type == ParameterSpecification.PT_INTEGER:
            try:
                ...
                raise BIValueError('Parameter "%s" should has integer value, but "%s" occured!'
    ...
    def __init__(self, *args, **kwargs):
        ...
        **kwargs - key-value attributes
        ...
        if (not args) and (not kwargs):
            raise BIException('Constructor of <%s> expect *args or **kwargs parameters' % self.__class__.__name__)
...
class CollectionSpecification(BaseSpecification):
    ...
    def validate(self):
        ...
        if not self.params_dict.get('allowed_types', None):
            raise BIValueError('Allowed resources types for collection "%s" is not specified!' % self.type_name())
...
class ConnectionSpecification(BaseSpecification):
    ...
    def validate(self):
        ...
        if not self.params_dict.get('connected_type', None):
            raise BIValueError('Connected resource type is not specified for connection "%s"!' % self.type_name())
...
class BaseEntity:
    ...
    def __init__(self, *args, **kwargs):
        ...
        **kwargs - key-value attributes
        ...
        if args:
            if len(args) != 1:
                raise Exception('Constructor of <%s> expect one no-named parameter only!' % self.__class__.__name__)
            params_dict = args[0]
